...
Zaim API を使用して一年分の支出明細を CSV ファイルにエクスポートするツールです。レシート画像のダウンロードも対応しています。
...
- **画像ファイル**: `./images-YYYYMMDD-HHMMSS/` ディレクトリに保存されます（実行時のタイムスタンプ付き）
  - 例: `./images-20250115-143022/`

**注意**: ファイル名とディレクトリ名には実行時のタイムスタンプが自動的に付与されるため、同じ期間で実行しても上書きされません。

### 画像の取得について

- Zaim API から取得した支出データに `image_url` フィールドが含まれている場合、自動的にレシート画像をダウンロードします
- 画像の取得には OAuth 認証を使用しているため、`.env` ファイルに正しい認証情報が設定されている必要があります
- 画像が存在しない取引や、画像URLが取得できない場合はスキップされます
